...
import com.borqs.qiupu.ui.circle.quickAction.SearchCategoryDialog;
...
//    private boolean mExpandedInActionView;
    ...
    private View mSearchDo;
    private View mSearchCancel;
    private TextView mSelectCategory;
    
    private SearchCategoryDialog mCategoryDialog;
...
//            	}
    ...
    public interface OnQueryTextListener {
         ...
         */
        boolean onQueryTextSubmit(String query, int searchType);
...
//        mSearchButton.setOnClickListener(mOnClickListener);
        ...
        mSearchDo = findViewById(R.id.search_do);
        mSearchDo.setOnClickListener(mOnClickListener);
        
        mSearchCancel = findViewById(R.id.search_cancel);
        
//        mLeftBack = findViewById(R.id.left_back);
        mSearchCancel.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				SearchView.this.setVisibility(View.GONE);
				mQueryTextView.setText("");
			}
		});
        
        mSelectCategory = (TextView) findViewById(R.id.left_select);
        mSelectCategory.setOnClickListener(mOnClickListener);
        
        mSearchType = BpcSearchActivity.SEARCH_TYPE_STREAM;
...
//        mSearchButton.setVisibility(visCollapsed);
    ...
    private void updateSubmitArea(boolean hasText) {
    	if(hasText) {
    		mSearchCancel.setVisibility(GONE);
    		mSearchDo.setVisibility(VISIBLE);
    	}else {
    		mSearchCancel.setVisibility(VISIBLE);
    		mSearchDo.setVisibility(GONE);
    	}
    }
...
//        invalidate();
    ...
    private final OnClickListener mOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            } else if(v.getId() == R.id.search_do) {
            	onSubmitQuery();
            }else if(v == mSelectCategory) {
...
//			if(mCategoryDialog == null) {
				...
				items.add(new SelectionItem(String.valueOf(BpcSearchActivity.SEARCH_TYPE_STREAM), 
						getResources().getString(R.string.search_post_title), String.valueOf(mSearchType)));
				items.add(new SelectionItem(String.valueOf(BpcSearchActivity.SEARCH_TYPE_CIRCLE), 
						getResources().getString(R.string.search_circles_title), String.valueOf(mSearchType)));
				items.add(new SelectionItem(String.valueOf(BpcSearchActivity.SEARCH_TYPE_PEOPLE),
						getResources().getString(R.string.search_people_title), String.valueOf(mSearchType)));
				mCategoryDialog = new SearchCategoryDialog(getContext(), SearchCategoryDialog.VERTICAL, items);
				mCategoryDialog.setListItemClickListener(categoryItemClickListener);
				mCategoryDialog.show(v);
//			}else {
//				mCategoryDialog.show(v);
//			}
		}
    ...
    OnItemClickListener categoryItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if(CorpusSelectionItemView.class.isInstance(view)) {
				CorpusSelectionItemView categoryItem = (CorpusSelectionItemView) view;
				SelectionItem item = categoryItem.getItem();
				if(item != null) {
					mSelectCategory.setText(item.getValue());
					mSearchType = Integer.parseInt(item.getId());
					mCategoryDialog.dismiss();
				}else {
					Log.d(TAG, "select item is null");
				}
			}
		}
	};
...
//    }
    ...
    private CharSequence getDecoratedHint(CharSequence hintText) {
        ...
        ssb.append(hintText);
        Drawable searchIcon = getContext().getResources().getDrawable(R.drawable.ic_search_api_holo_light);
...
//        	Log.d(TAG, "IntentUtil onEditorAction: " + actionId + " " + event.getAction());
            onSubmitQuery();
            return true;
        }
    };
...
//        updateSubmitArea();
    ...
    private void onSubmitQuery() {
        ...
        Log.d(TAG, "IntentUtil onSubmitQuery : " + query);
        if (query != null && TextUtils.getTrimmedLength(query) > 0) {
            if (mOnQueryChangeListener == null
                    || !mOnQueryChangeListener.onQueryTextSubmit(query.toString(), mSearchType)) {
                if (mSearchable != null) {
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
    	Log.d(TAG, "onVisibilityChanged: " + changedView + " " + visibility);
    	if(getClass().isInstance(changedView) && visibility == View.VISIBLE) {
    		onSearchClicked();
    	}else {
    		setImeVisibility(false);
    	}
...
//        	mCloseButton.setVisibility(s.toString().length() > 0 ? View.VISIBLE : View.GONE);
        }
    };
    ...
    public static class SearchAutoComplete extends AutoCompleteTextView {
        ...
        public SearchAutoComplete(Context context, AttributeSet attrs) {
            super(context, attrs);
            mThreshold = getThreshold();
            setHint(getResources().getString(R.string.search_all_hint));
        }
