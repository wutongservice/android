...
public class TwoWayGridView extends TwoWayAbsListView {
	...
	private class HorizontalGridBuilder extends GridBuilder {
        ...
        private View makeRowAsColumn(int startPos, int x, boolean flow) {
            if (DEBUG)
                Log.i(TAG, "makeRowAsColumn() startPos: " + startPos + " x: " + x + " flow: " + flow + " mFirstPosition: " + mFirstPosition);
            final int rowHeight = mRowHeight;
            final int verticalSpacing = mVerticalSpacing;

            int last;
            int nextTop = mListPadding.left +
            ...
            if (!mStackFromBottom) {
                ...
                if (last - startPos < mNumColumns) {
                    nextTop += (mNumColumns - (last - startPos)) * (rowHeight + verticalSpacing);
                }
            }
        ...
        private View makeColumn(int startPos, int x, boolean flow) {
            if (isHorizontalScrollColumn()) {
                return makeRowAsColumn(startPos, x, flow);
            }
		...
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (isHorizontalScrollColumn()) {
                determineColumnAsRows(childHeight);
            } else {
        ...
        private boolean isHorizontalScrollColumn() {
            if (STRETCH_COLUMN_WIDTH == mStretchMode &&
                    !mPortraitOrientation && !mScrollVertically) {
                return true;
            }
        ...
        private void determineColumnAsRows(int availableSpace) {
            final int stretchMode = mStretchMode;
            if (stretchMode != STRETCH_COLUMN_WIDTH) {
                Log.w(TAG, "determineColumnAsRows, unrecognized mode: " + stretchMode);
                determineRows(availableSpace);
            } else {
                final int requestedVerticalSpacing = mRequestedHorizontalSpacing;
                final int requestedRowHeight = mRequestedColumnWidth;
                mHorizontalSpacing = mRequestedVerticalSpacing;

                if (mRequestedNumColumns == AUTO_FIT) {
                    if (requestedRowHeight > 0) {
                        // Client told us to pick the number of rows
                        mNumRows = (availableSpace + mRequestedHorizontalSpacing) /
                                (requestedRowHeight + mRequestedHorizontalSpacing);
                    } else {
                        // Just make up a number if we don't have enough info
                        mNumRows = 2;
                    }
