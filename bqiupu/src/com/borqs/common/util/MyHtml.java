...
public class MyHtml {
	...
	public static String toDumbClickableText(String s) {
        return String.format("<a href='borqs://myhtml/dumb?noneedtakeaction=true'>%1$s</a>", s);
    }
    ...
    public static Spanned fromHtml(String source, Context con) {
        return fromHtml(source, null, null, con);
    }
    ...
    public static Spanned fromHtml(String source, ImageGetter imageGetter,
            TagHandler tagHandler, Context con) {
    	...
    	if(con != null)
    	{
    		...
    		mContext = new WeakReference<Context>(con.getApplicationContext());
    	}
    ...
    public static Spanned fromHtml(String source, ImageGetter imageGetter,
    	...
    	return fromHtml(source, imageGetter, tagHandler, null);
    }
...
class HtmlToSpannedConverter implements ContentHandler {
    ...
    public HtmlToSpannedConverter(
            String source, MyHtml.ImageGetter imageGetter, MyHtml.TagHandler tagHandler,
            Parser parser) {
        mSource = source;
    ...
    private static void startImg(SpannableStringBuilder text,
                                 Attributes attributes, MyHtml.ImageGetter img) {
        ...
        if (d == null) {
        	if(MyHtml.mContext != null && MyHtml.mContext.get() != null)
        	{
	            d = MyHtml.mContext.get().getResources().
	                    getDrawable(com.borqs.qiupu.R.drawable.unknown_image);
    ...
    public static class BorqsURLSPan extends URLSpan {
        ...
        public BorqsURLSPan(String src) {
            super(src);
            url = src;
        }
        ...
        public void updateDrawState(TextPaint ds) {
        	...
        	ds.linkColor = (Color.parseColor("#ef333333"));        	
            ds.setColor(Color.parseColor("#ef3B5998"));        	
        ...
        public void onClick(View widget) {
            if (null != widget && widget instanceof TextView) {
                ...
                // http://bpc.borqs.com/login.html#showdetail/2840043923409216759/false
                if(uri.getHost() != null && (uri.getHost().contains("bpc.borqs.com") || uri.getHost().contains("wutong.borqs.com") ))
                {
                	//find stream id
                	final String url = getURL();
                	if(url.contains("showdetail"))
                	{
                		String[] sep = url.split("/");
                		
                		//Stream id = the last second id
                		try{
                		    String sid = sep[sep.length-2];
                		    
                		    //open stream details view
                		    String streamuri  = BpcApiUtils.STREAM_COMMENT_SCHEME_PATH + "?" + BpcApiUtils.SEARCH_KEY_ID + "=" + sid;
                		    Intent intent = new Intent(Intent.ACTION_VIEW);
                		    intent.setData(Uri.parse(streamuri));
                            textView.getContext().startActivity(intent);
//                		    IntentUtil.startStreamCommentIntentByStreamID(textView.getContext(), sid);
                		    return ;
                		}catch(Exception ne){
                			//continue to go
                		}
                ...
                if (charSequence instanceof SpannableString) {
                    ...
                    if (owner != null) {
                        ...
                        textView.getContext().startActivity(intent);
                        */
                        if(SNSItemView.isURL(getURL())) {
                        	IntentUtil.startLinkTraActivity(widget.getContext(), getURL());
                        }else {
                ...
                else
                {
                    if(SNSItemView.isURL(getURL())) {
                    	...
                    	if (BpcApiUtils.isActivityReadyForIntent(context, intent)) {
                    		if(EventDetailActivity.class.isInstance(context)) {
                    			EventDetailActivity event = (EventDetailActivity) context;
                    			String id = BpcApiUtils.parseSchemeValue(intent, BpcApiUtils.SEARCH_KEY_UID);
                    			if(!TextUtils.isEmpty(id) && event.getEventId() == Long.parseLong(id)) {
                    				return ;
                    			}
                    			context.startActivity(intent);
                    		}else if(UserPublicCircleDetailActivity.class.isInstance(context)) {
                    			UserPublicCircleDetailActivity circle = (UserPublicCircleDetailActivity) context;
                    			String id = BpcApiUtils.parseSchemeValue(intent, BpcApiUtils.SEARCH_KEY_UID);
                    			if(!TextUtils.isEmpty(id) && circle.getCircleId() == Long.parseLong(id)) {
                    				return ;
                    			}
                    			context.startActivity(intent);	
                    		...
                    		Log.i("MyHtml", "ignore click while no activity for intent: " + intent);
                    	}
                    }
    ...
    private static void endA(SpannableStringBuilder text) {
        ...
        if (where != len) {
            ...
            if (h.mHref != null) {
                text.setSpan(new BorqsURLSPan(h.mHref), where, len,
                             Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                
                //text.setSpan(new RelativeSizeSpan(1.5f), where, len, 0);  
            }
        }
    }
