...
public class HtmlUtils {
    // replace from user nickname that includes SPACE char, which
    // causes link function lost within html presentation.
    // 1. replace all space char within the user name by "&nbsp;"
    // 2. excludes the space char in 'href' segment.
    public static String text2html(String text) {
        ...
        if (last > index) {
            String tmpname = text.substring(index, last);
            tmpname = tmpname.replace(" ", "&nbsp;");
            text = text.substring(0, index) + tmpname + text.substring(last, text.length());
        }
        return text;
    }
